    ...
    setup(e) {
      ...
      St(
        ...
        (h) => {
          if (t.value) {
            let y = D(h);
            browser.storage.local.set({ images: y });
          }
          n.length < 1 ? (h.isMainImage = !0) : (h.isMainImage = !1),
      ...
      async function g() {
        ...
        if ((console.log(T.theme), T.theme === "custom-theme")) {
          let P = await browser.storage.local.get("customBgColor"),
            F = await browser.storage.local.get("customAccentColor");
        ...
        let E = browser.storage.local.get("currentFont"),
          k;
        if (
          (await E.then((P) => {
          ...
          let P = document.querySelector("main"),
            F = await browser.storage.local.get("currentFont");
          P.classList.add(F.currentFont);
        }
        let U = await browser.storage.local.get("popupSize");
        const R = (P) => {
